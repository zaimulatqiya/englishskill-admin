...
const generateCertificateCanvas = async (profile) => {
  ...
  content.innerHTML = `
    <div style="${smallTextStyle} left: ${mmToPx(7)}px; width: ${mmToPx(79)}px; top: ${adjustY(84)}px; text-align: center;">
      ${formatDate(profile.expired || formatDatePlusYears(Date.now(), 2))}
    </div>
    <div style="${regisTextStyle} left: ${mmToPx(232)}px; top: ${adjustY(81)}px;">
      ${profile.nomor_registrasi}/${formatDateMMDDYYYY(profile.tanggal_selesai_ujian)}
    </div>
    ...
    <div style="${tanggalCetakStyle} left: ${mmToPx(146.5)}px; top: ${adjustY(159)}px;">
      Pare, ${formatDate(profile.tanggal_cetak_sertifikat || Date.now())}
    </div>
